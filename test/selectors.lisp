...
(deftest test-parser-speed (parser compiler)
  (iter (for i from 0 to 100)
	(iter (for (n selector) in +nodes-and-matching-selectors+)
	      (assert-true (list n (parse-results selector))))))

(deftest test-compiler1-speed (compiler compilation-speed)
  (iter (for i from 0 to 100)
	(iter (for (n selector) in +nodes-and-matching-selectors-parsed+)
	      (assert-true (compile-css-node-matcher selector)
			   n selector))))

(deftest test-compiler2-speed (compiler compilation-speed)  
  (iter (for i from 0 to 100)
	(iter (for (n selector) in +nodes-and-matching-selectors-parsed+)
	      (assert-true (make-node-matcher selector)
				 n selector))))

(deftest test-compiler1-execution-speed (matcher compiler execution-speed)
...
(deftest test-compiler2-execution-speed (matcher compiler execution-speed)
  (iter (for i from 0 to 1000)
